...
int main() {
	...
	bot.on_message_create([&bot, adminid, &voiceall, &Servers, &beforetimetts](const dpp::message_create_t& event) {
		...
		if (messagel.substr(0, 8) == to_utf8(L"ранд")) {
			...
			if (args[1] != to_utf8(L"список")) {
				int num = -1;
				try {
					num = std::stoull(args[1]);
				}
				catch (...) {
				...
				reply = to_utf8(L"Выпало: **") + args[getRandomNumber(gen, 0, args.size() - 3) + 2] + "**";
				event.reply(reply);
			}
		}
		...
		if (messagel.substr(0, 12) == to_utf8(L"лидеры") and author_id != bot.me.id) {
			...
			while (file >> leadstr >> exp) {
				leaders.emplace_back(std::stoull(keep_digits(leadstr)), exp);
			}
